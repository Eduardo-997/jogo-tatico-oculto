...
__refRoot.GameReferee = class GameReferee {
  ...
  #finishActivation(side){
    const p=this.#activePiece(side);if(p&&p.alive){p.activated=true;this.#tickPieceEffects(p,'activation');}this.#s.activation[side]=null;
  ...
  #advanceAfterActivation(side){
    ...
    if(!pLeft&&!eLeft){this.#s.round++;this.#tickRoundEffects();for(const p of this.#pieces('player'))if(p.alive)p.activated=false;for(const p of this.#pieces('enemy'))if(p.alive)p.activated=false;this.#addHistory('player',`🔄 Rodada ${this.#s.round} começou.`);this.#addHistory('enemy',`🔄 Rodada ${this.#s.round} começou.`);this.#s.turn='player';return;}
...
export class TriGameRoom {
  constructor(ctx,env){
    this.ctx=ctx;this.env=env;this.referee=new TriReferee();this.ready={A:null,B:null};this.started=false;this.ai=null;this.difficulty='normal';
    ctx.blockConcurrencyWhile(async()=>{const saved=await ctx.storage.get('triRoom');if(saved){this.ready=saved.ready||{A:null,B:null};this.started=!!saved.started;this.difficulty=saved.difficulty||'normal';if(saved.gameState)this.referee.importState(saved.gameState);if(this.started)this.ai=new TriAI('C',this.difficulty);}});
  }
  async persist(){await this.ctx.storage.put('triRoom',{ready:this.ready,started:this.started,difficulty:this.difficulty,gameState:this.referee.exportState()});}
  send(ws,obj){try{ws.send(JSON.stringify(obj));}catch{}}
  sockets(){return this.ctx.getWebSockets();}
  attachment(ws){try{return ws.deserializeAttachment()||{};}catch{return {};}}
  sideSocket(side){return this.sockets().find(ws=>this.attachment(ws).side===side)||null;}
  ...
  broadcast(obj){for(const ws of this.sockets())this.send(ws,obj);}
  broadcastRoomState(){this.broadcast(this.roomState());}
  broadcastViews(){if(!this.started)return;for(const side of ['A','B']){const ws=this.sideSocket(side);if(ws)this.send(ws,{type:'view',view:this.referee.client(side).getView()});}}
  async runAI(){if(!this.started||!this.ai)return;let guard=0;while(guard++<80){const v=this.referee.client('C').getView();if(v.gameOver)break;if(!(v.pendingCombat||v.doppelChoice||v.turn==='C'))break;const act=this.ai.decide(v);if(!act||act.type==='wait')break;applyTriAction(this.referee.client('C'),act);this.broadcastViews();await new Promise(r=>setTimeout(r,120));}}
  ...
  async webSocketMessage(ws,message){
    ...
    if(msg.type==='join'){
      ...
      if(!chosen){this.send(ws,{type:'error',message:'Sala 1x1x1 cheia: já existem dois humanos.'});try{ws.close(1008,'Sala cheia');}catch{}return;}
      ws.serializeAttachment({side:chosen});this.send(ws,{type:'joined',room:String(msg.room||''),side:chosen});this.broadcastRoomState();if(this.started)this.broadcastViews();return;
    }
    if(!side)return this.send(ws,{type:'error',message:'Entre na sala primeiro.'});
    if(msg.type==='ping')return this.send(ws,{type:'pong'});
    if(msg.type==='ready'){
      ...
      if(this.ready.A&&this.ready.B){this.difficulty=['easy','normal','hard'].includes(this.ready.A.difficulty)?this.ready.A.difficulty:'normal';const st=this.referee.startOnline(this.ready.A.setup,this.ready.A.bases,this.ready.B.setup,this.ready.B.bases,this.difficulty);if(!st.ok)return this.broadcast({type:'result',ok:false,status:st.status});this.started=true;this.ai=new TriAI('C',this.difficulty);}
      await this.persist();this.broadcastRoomState();this.broadcastViews();return;
    }
    if(msg.type==='action'){
      if(!this.started)return this.send(ws,{type:'result',ok:false,status:'A partida ainda não começou.'});let res;try{res=applyTriAction(this.referee.client(side),msg.action||{});}catch(e){console.error(e);res={ok:false,status:'Erro interno.'};}
      this.send(ws,{type:'result',...res});this.broadcastViews();await this.runAI();await this.persist();this.broadcastViews();return;
    }
