...
for(const [di,difficulty]of ['easy','normal','hard','extreme'].entries())for(let game=0;game<Number(process.env.GAMES||20);game++){
  ...
  let ref=new globalThis.GameReferee();const team=(offset,coords)=>Array.from({length:4},(_,i)=>({name:R.defs[(game*4+di*3+offset+i)%20].name,coord:coords[i]}));
  const start=ref.startMultiplayerGame(team(0,['A2','C2','E2','H2']),['B1','G1'],team(7,['H7','F7','D7','A7']),['G8','B8']);if(!start.ok)throw Error(start.status);
  const control=defaultGeneralControl();control.difficulties={player:difficulty,enemy:difficulty};let brains=makeGeneralBrains(control),steps=0;
  for(;steps<5000;steps++){
    const raw=JSON.parse(ref.exportState());if(raw.gameOver)break;
    const step=generalStep(ref,brains);out.actions++;
    ...
    if(process.env.RESTORE==='1'&&steps%31===0){const state=ref.exportState(),saved=brainSnapshots(brains);const bytes=Buffer.byteLength(JSON.stringify({gameState:state,generalBrains:saved,generalControl:control,ready:{player:team(0,['A2','C2','E2','H2']),enemy:team(7,['H7','F7','D7','A7'])}}));out.maxCheckpointBytes=Math.max(out.maxCheckpointBytes,bytes);if(bytes>120000)throw Error('Checkpoint próximo do limite de armazenamento: '+bytes);ref=new globalThis.GameReferee();ref.importState(state);brains=makeGeneralBrains(control,saved);}
  }
