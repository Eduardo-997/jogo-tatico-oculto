(function(){
const {createClassicBrain}=window.ClassicBrains;
const GENERAL_SIDES=['player','enemy'];
const defaultGeneralControl=()=>({paused:false,delay:600,difficulties:{player:'normal',enemy:'normal'},error:null});
function makeGeneralBrains(control,saved={}){return Object.fromEntries(GENERAL_SIDES.map(side=>[side,createClassicBrain(side,control.difficulties[side],saved?.[side])]));}
const brainSnapshots=brains=>Object.fromEntries(GENERAL_SIDES.filter(side=>brains?.[side]).map(side=>[side,brains[side].snapshot()]));
function finishGeneralObservation(referee,reason='manual'){
...
function applyGeneralRecord(referee,entry){return entry.action?.type==='observerFinish'?finishGeneralObservation(referee,entry.action.reason||'manual'):applyGeneralAction(referee.createClient(entry.side),entry.action);}
function describeGeneralStep(raw,action,result,side,after){
...
function applyGeneralAction(client,a){
  switch(a?.type){
...
function generalStep(referee,brains,control={}){
...
window.GeneralGame={GENERAL_SIDES,defaultGeneralControl,makeGeneralBrains,brainSnapshots,finishGeneralObservation,applyGeneralRecord,describeGeneralStep,applyGeneralAction,generalStep};
})();
