...
(() => {
  ...
  function renderTrainingActionControls(vs){
    ...
    setActionVisual(attack,Number(p.a||0)>0||(p.name==='Fantasma'&&!p.possessing),false);
  ...
  function paintGame(vs=views()){
    ...
    for(const [c,b] of cells){
      ...
      if(p&&mode){
        ...
        if(mode==='pyro'&&R.abilityCells(p).includes(c)&&!baseAtState(state,c))b.classList.add('attack-highlight');if(mode==='sureShotConfirm'&&R.attackCells({...p,range:(p.range||0)*2}).includes(c)&&!baseAtState(state,c))b.classList.add('attack-highlight');if(mode==='paranoiaPresence'&&R.abilityCells(p,true).includes(c))b.classList.add('highlight');
    ...
    for(const c of new Set(['player','enemy'].flatMap(s=>vs[s].impactCells||[vs[s].impactCell].filter(Boolean))))addFieldMarker(cells.get(c),Assets.effects?.dano,'impact','Ataque ocorreu aqui','💥');
  ...
  function handleChoices(vs){
    ...
    if(dc){const d=vs[dc].doppelChoice;showPopup(`<b>🎭 Doppelgänger — Lado ${sideName(dc)}</b><div class="muted small" style="margin:5px 0 9px">Manter ${d.current} ou copiar ${d.newAbility}? ${d.reason||''}</div><div class="row"><button data-keep>Manter</button><button class="primary" data-copy ${d.canCopyNew===false?'disabled':''}>Copiar nova</button></div>`);popup.querySelector('[data-keep]').onclick=()=>after(client(dc).chooseDoppelCopy(false));popup.querySelector('[data-copy]').onclick=()=>after(client(dc).chooseDoppelCopy(true));return;}
