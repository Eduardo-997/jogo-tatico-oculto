...
__gameRoot.GameRules = (() => {
  ...
  const branchDef={name:'Galho-Vivo',icon:'🌲',type:'R',typeIcon:'🪨',v:2,m:1,a:1,range:1,per:1,ah:0};
  ...
  const baseBonuses=[
    ...
    {id:'abilityRange',icon:'✨',name:'Canalização',description:'+1 AH (Alcance de Habilidade) permanente para uma unidade cuja habilidade use alcance.'}
  ];
  ...
  function attackCells(p){
    const d=defOf(p),out=[];
    if(d.a<=0 && p.name!=='Fantasma')return out;
  ...
  function abilityCells(p){
    const d=defOf(p),out=[];
    for(let y=0;y<8;y++)for(let x=0;x<8;x++){const c=coord(x,y);if(c!==p.coord&&man(p.coord,c)<=d.ah)out.push(c)}
    return out;
  }
