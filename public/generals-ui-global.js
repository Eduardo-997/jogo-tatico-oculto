...
function startChosenFormations(){
  if(mode!=='local'||state)return;
  const testRef=new window.GameReferee();
  for(const s of sides){const d=drafts[s],result=testRef.validateSetup(s,d.setup,d.bases,teamSize(s));if(!result.ok){side=s;$('generalSide').value=s;status(`${label(s)} ainda não está pronto: ${result.status}`);render();return;}}
  ready={player:true,enemy:true};ref=new window.GameReferee();const a=drafts.player,b=drafts.enemy,start=ref.startMultiplayerGame(a.setup,a.bases,b.setup,b.bases,matchConfig);
  if(!start.ok){ready={player:false,enemy:false};status(start.status);render();return;}
  brains=makeGeneralBrains(control);state=JSON.parse(ref.exportState());recorder=new window.GameReplay.Recorder(()=>ref.exportState());capture('Início da partida');status('Formações confirmadas. As IAs assumiram o comando.');saveLocal();render();schedule();
}
...
function render(){
  ...
  $('generalManualStart').hidden=mode!=='local';$('generalManualStart').disabled=!!state;$('generalQuickWatch').hidden=mode!=='local';$('generalQuickWatch').disabled=!!state;
  ...
  const roster=$('generalRoster');roster.replaceChildren();for(const d of R.defs){const chosenNow=drafts[side].setup.some(p=>p.name===d.name),active=tool?.kind==='piece'&&tool.name===d.name,b=node('button',null,(chosenNow?'chosen ':'')+(active?'active-target':'')),copy=node('span',null,'general-roster-copy');b.type='button';b.disabled=locked();b.setAttribute('aria-pressed',String(chosenNow));copy.append(node('b',d.name),node('small',`V${d.v} · M${d.m} · ATQ${d.a} · ${R.archetypeName(d.type)}`));b.append(image(A.character(d.name),'',d.name),copy);b.title=`${d.name} · V${d.v} M${d.m} ATQ${d.a} · ${R.archetypeName(d.type)}`;b.onclick=()=>selectPiece(d.name);roster.appendChild(b);}
...
$('generalManualStart').onclick=startChosenFormations;
